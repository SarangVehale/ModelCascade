...
def pull_model(model_name):
    ...
    while retries < MAX_RETRIES:
        if skip_model:
            ...
            return "skipped"
        if exit_script:
            logging.warning(f"Exiting script (user request)")
            return "exited"
        try:
...
def listen_for_input():
    """
    Listen for user input to skip the current model or exit the script.
    """
    global skip_model, exit_script
    while True:
...
def main():
    ...
    print(Fore.YELLOW + "Press 'e' and Enter to exit the script.\n")
    try:
    ...
    if exit_script:
        print(Fore.RED + "🚪 Script exited by user.")
